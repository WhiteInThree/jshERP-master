...
public interface BudgetSettingMapper {
    ...
    @Select("select b.id,b.budget_year budgetYear,o.id organizationId,o.org_abr organizationName,ifnull(b.budget_amount,0) budgetAmount,ifnull(u.used_amount,0) usedAmount,(ifnull(b.budget_amount,0)-ifnull(u.used_amount,0)) availableAmount from jsh_organization o left join jsh_budget_setting b on b.organization_id=o.id and b.budget_year=#{year} and b.tenant_id=#{tenantId} and ifnull(b.delete_flag,'0')!='1' left join (select a.organ_id organization_id,sum(ifnull(di.all_price,0)) used_amount from jsh_depot_head dh join jsh_depot_item di on di.header_id=dh.id and ifnull(di.delete_flag,'0')!='1' join jsh_depot_head a on a.number=dh.link_number and a.type='其它' and a.sub_type='请购单' and a.status='4' and ifnull(a.delete_flag,'0')!='1' where dh.type='出库' and dh.sub_type='其它' and ifnull(dh.delete_flag,'0')!='1' and dh.tenant_id=#{tenantId} and year(dh.oper_time)=#{year} group by a.organ_id) u on u.organization_id=o.id where o.id=#{organizationId} and o.tenant_id=#{tenantId} and ifnull(o.delete_flag,'0')!='1' limit 1")
    BudgetSettingVo findByOrganization(@Param("year") Integer year, @Param("organizationId") Long organizationId, @Param("tenantId") Long tenantId);

    @Select("select b.id,b.budget_year budgetYear,o.id organizationId,o.org_abr organizationName,ifnull(b.budget_amount,0) budgetAmount,ifnull(u.used_amount,0) usedAmount,(ifnull(b.budget_amount,0)-ifnull(u.used_amount,0)) availableAmount from jsh_budget_setting b join jsh_organization o on o.id=b.organization_id and o.tenant_id=b.tenant_id and ifnull(o.delete_flag,'0')!='1' left join (select a.organ_id organization_id,sum(ifnull(di.all_price,0)) used_amount from jsh_depot_head dh join jsh_depot_item di on di.header_id=dh.id and ifnull(di.delete_flag,'0')!='1' join jsh_depot_head a on a.number=dh.link_number and a.type='其它' and a.sub_type='请购单' and a.status='4' and ifnull(a.delete_flag,'0')!='1' where dh.type='出库' and dh.sub_type='其它' and ifnull(dh.delete_flag,'0')!='1' and dh.tenant_id=#{tenantId} and year(dh.oper_time)=#{year} group by a.organ_id) u on u.organization_id=b.organization_id where b.budget_year=#{year} and b.tenant_id=#{tenantId} and ifnull(b.delete_flag,'0')!='1' and ifnull(b.budget_amount,0)>0 and (ifnull(b.budget_amount,0)-ifnull(u.used_amount,0)) < b.budget_amount * 0.2 order by availableAmount asc limit 5")
    List<BudgetSettingVo> findWarnings(@Param("year") Integer year, @Param("tenantId") Long tenantId);
