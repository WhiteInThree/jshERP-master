...
public class DepotHeadService {
    ...
    public String getCreatorByCurrentUser() throws Exception {
        ...
        if(!userService.isAdminOrOfficeUser(user)) {
    ...
    private boolean isDepartmentLimitedUser(User user) {
        return !userService.isAdminOrOfficeUser(user);
    }

    private void checkDepartmentPurchaseApplyCreate(User user, DepotHead depotHead) {
        if(!isDepartmentLimitedUser(user)) {
            return;
        }
        if(!BusinessConstants.DEPOTHEAD_TYPE_OTHER.equals(depotHead.getType())
            ...
            throwDepartmentPermissionException();
        }
    ...
    private void checkDepartmentPurchaseApplyOwner(User user, DepotHead depotHead) {
        if(!isDepartmentLimitedUser(user)) {
            return;
        }
        if(depotHead == null
                || !BusinessConstants.SUB_TYPE_PURCHASE_APPLY.equals(depotHead.getSubType())
                || depotHead.getCreator() == null
                || !depotHead.getCreator().equals(user.getId())) {
            throwDepartmentPermissionException();
        }
    }

    private void checkDepartmentCanAudit(User user) {
        if(isDepartmentLimitedUser(user)) {
            throwDepartmentPermissionException();
        }
    }

    private void throwDepartmentPermissionException() {
        throw new BusinessRunTimeException(ExceptionConstants.DEPOT_HEAD_EDIT_FAILED_CODE,
                "\u666e\u901a\u90e8\u95e8\u8d26\u53f7\u53ea\u80fd\u64cd\u4f5c\u672c\u90e8\u95e8\u81ea\u5df1\u521b\u5efa\u7684\u8bf7\u8d2d\u5355");
    }
    ...
    public int batchDeleteBillByIds(String ids)throws Exception {
        ...
        for(DepotHead depotHead: dhList){
            checkDepartmentPurchaseApplyOwner(currentUser, depotHead);
            //只有未审核的单据才能被删除
    ...
    public int batchForceClose(String ids, HttpServletRequest request) throws Exception {
        int result = 0;
        checkDepartmentCanAudit(userService.getCurrentUser());
    ...
    public void addDepotHeadAndDetail(String beanJson, String rows,
        ...
        //判断用户是否已经登录过，登录过不再处理
        User userInfo=userService.getCurrentUser();
        checkDepartmentPurchaseApplyCreate(userInfo, depotHead);
        //通过redis去校验重复
    ...
    public void updateDepotHeadAndDetail(String beanJson, String rows,HttpServletRequest request)throws Exception {
        ...
        checkDepartmentPurchaseApplyOwner(userInfo, oldDepotHead);
        checkDepartmentPurchaseApplyCreate(userInfo, depotHead);
        //通过redis去校验重复
        checkExistByRedis(userInfo, depotHead);
        //校验单号是否重复
    ...
    public void batchAddDepotHeadAndDetail(String ids, HttpServletRequest request) throws Exception {
        ...
        User userInfo=userService.getCurrentUser();
        checkDepartmentCanAudit(userInfo);
        for(DepotHead depotHead : dhList) {
    ...
    public void quickEditDepotHead(Long id, String remark, HttpServletRequest request) throws Exception {
        try {
            ...
            User currentUser = userService.getCurrentUser();
            checkDepartmentPurchaseApplyOwner(currentUser, oldDepotHead);
            if(isDepartmentLimitedUser(currentUser) && !BusinessConstants.BILLS_STATUS_UN_AUDIT.equals(oldDepotHead.getStatus())) {
                throw new BusinessRunTimeException(ExceptionConstants.DEPOT_HEAD_BILL_CANNOT_EDIT_CODE,
                        String.format(ExceptionConstants.DEPOT_HEAD_BILL_CANNOT_EDIT_MSG));
            }
            DepotHead depotHead = new DepotHead();
