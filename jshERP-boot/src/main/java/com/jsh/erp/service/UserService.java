...
public class UserService {
     ...
     * @throws Exception
     */
    public boolean isAdminOrOfficeUser(User user) {
        if(user == null) {
            return false;
        }
        try {
            Role role = user.getId() == null ? null : getRoleTypeByUserId(user.getId());
            if(role == null || StringUtil.isEmpty(role.getValue())) {
                return false;
            }
            String roleCode = role.getValue();
            return BusinessConstants.ROLE_CODE_ADMIN.equals(roleCode)
                    || BusinessConstants.ROLE_CODE_OFFICE.equals(roleCode);
        } catch (Exception e) {
            JshException.readFail(logger, e);
            return false;
        }
    ...
    public JSONArray getBtnStrArrById(Long userId) throws Exception {
        ...
        boolean deptLimited = !isAdminOrOfficeUser(currentUser);
        ...
        if(btnStrArr.size()>0) {
            ...
            for (Object obj : btnStrArr) {
                ...
                if(deptLimited && !isDepartmentAllowedFunctionUrl(url)) {
                    continue;
                }
    ...
    private boolean isDepartmentAllowedFunctionUrl(String url) {
        return StringUtil.isNotEmpty(url) && (url.contains("/bill/purchase_apply") || url.contains("/report"));
    }
